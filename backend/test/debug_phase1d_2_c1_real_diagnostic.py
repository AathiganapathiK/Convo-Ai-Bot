...
def main():
    ...
    print("=" * 80)
    print("REAL-DATA AMBIGUITY CLASSIFICATION DIAGNOSTIC")
    print("=" * 80)
    ...
    for q in QUERIES:
        ...
        for idx, c in enumerate(res.candidates, 1):
            print(f"  #{idx} {c.value}")
            print(f"      Match Type           : {c.match_type.value}")
            print(f"      Confidence           : {c.confidence:.2f}")
            print(f"      Actual Query Coverage: {c.actual_query_coverage}")
            print(f"      Matched Query Tokens : {c.matched_query_tokens}")
            print(f"      Dimension            : ID={c.dimension_id}, Name={c.business_name}, Table={c.table_name}, Col={c.column_name}")
